...
| `ollama-cloud` | Ollama Cloud | `providers/ollama-cloud.js` | OpenCode `auth.json` `ollama-cloud` entry (API key under `key` or `token`), else the manual cookie pasted into Settings (`aid=...; __Secure-session=...` from `ollama.com`) stored under `~/.config/openchamber/quota/` |
...
## Ollama Cloud usage sources

Ollama Cloud reads usage from exactly one source per refresh, in strict priority order; sources are never merged.

- **OpenCode API key (primary)** — when the OpenCode `auth.json` has an `ollama-cloud` entry (`{ key, type: "api" }`; a non-empty `token` also works), usage comes from the undocumented `GET https://ollama.com/api/usage` with `Authorization: Bearer <key>`. `limits.session.usage` and `limits.weekly.usage` are 0..1 fractions of the window consumed; a missing, blank, boolean, or non-finite value omits that window instead of rendering 0%, and values are clamped to 0..100% without rounding. The `activity` block and `limits.*.models` breakdowns are ignored. A payload with no session/weekly fractions (for example a cost-based plan the endpoint does not describe) reports `Ollama Cloud usage API returned no usage limits for this plan` — a distinct failure, not a parse error.
- **Managed cookie (fallback)** — used only while no API key exists. A key that exists makes the API the only source: HTTP, redirect, auth, and parse failures are reported as failures and never fall back to the cookie. Because the sources are not merged, cookie-only windows (`premium`, `monthly`, `credits_balance`) are absent from API-key results. An unreadable or corrupt `auth.json` counts as "no API key".

Ollama Cloud authentication uses two cookies (`aid` and `__Secure-session`) pasted together as one single-line Cookie header value. Ollama serves two different `/settings` page shapes and `parseOllamaSettingsHtml` supports both: session/weekly/premium-interaction windows (percent-based plans) and a `monthly` window derived from "Monthly usage: $X of $Y used" (cost-based plans) with a symmetric `$X / $Y` money `valueLabel` that reads correctly in both used/remaining display modes. The "Extra usage" credits block is surfaced as a balance-only `credits_balance` window with a plain money `valueLabel` when present, matching the Codex/DeepSeek credits treatment ("Credits Balance" in the UI); a $0 balance is omitted instead of showing an empty credits row. Keep `packages/web/server/lib/quota/providers/ollama-cloud.js` and `packages/vscode/src/quotaProviders.ts`/`packages/vscode/src/ollamaQuota.ts` (`parseOllamaSettingsHtml`, `parseOllamaUsageJson`, `fetchOllamaApiUsage`/`fetchOllamaUsageApi`) in sync — the VS Code extension duplicates this parsing logic rather than importing it.
