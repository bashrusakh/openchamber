...
- Working-tree comparisons use the real index read-only. When untracked paths exist, a temporary copy of the index receives intent-to-add entries so Git computes additions, deletions, recreations, and renames together. Current contents come from the working tree, symlinks remain links, ignored files stay excluded, and temporary files are removed after success or a confirmed process cleanup. An unconfirmed process-tree cleanup retains the temporary directory and its termination metadata so a held Git process cannot lose its index or release its read lease early.
