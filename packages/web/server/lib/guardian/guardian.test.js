...
describe('ManagedOpenCodeGuardian', () => {
  ...
  it.skipIf(process.platform === 'win32')('adopt transitions to Claimed with valid capability', async () => {
    const mockChild = createMockChild();
    const spawnFn = vi.fn().mockReturnValue(mockChild);
    const { guardian, secretProvider } = await createGuardianFixture({ spawnFn });
    ...
    const { incarnation } = await spawnPromise;
    await guardian.prepareHandoff({ incarnation });

    // Issue a lifecycle credential to use as claim capability.
    const material = await secretProvider.issueLifecycleCredential({ incarnation });
    let claimCapability;
    await material.withCredential(async (credential) => {
      claimCapability = credential.toString('base64url');
    });

    const claimed = await guardian.adopt({ incarnation, claimCapability });
    expect(claimed.state).toBe(ManagedOpenCodeHandoffV2State.Claimed);

    material.dispose();
    await guardian.stop();
  });

  it.skipIf(process.platform === 'win32')('adopt fails with invalid capability', async () => {
    const mockChild = createMockChild();
    const spawnFn = vi.fn().mockReturnValue(mockChild);
    const { guardian } = await createGuardianFixture({ spawnFn });
    ...
    await expect(guardian.adopt({ incarnation, claimCapability: 'invalid' }))
      .rejects.toThrow('Invalid claim capability');
