...
- `POST /api/fs/clone` reserves the canonical destination through the shared Git execution coordinator. The reservation covers clone processing and partial-clone cleanup; clone children own a detached process group/tree, so cancellation and timeout await descendant termination before destination cleanup and scheduler release. Network capacity is released after the full clone termination lifecycle completes.
