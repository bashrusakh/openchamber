...
- `packages/web/server/lib/opencode/proxy.js`: OpenCode API/SSE forwarding, the readiness gate, and the consult-reservation prompt gate (POST `/session/<id>/{prompt_async,message,prompt,command,shell}` → 409 `consult-reservation` while the message queue holds a live consult claim; fail-open, other methods/routes untouched).
