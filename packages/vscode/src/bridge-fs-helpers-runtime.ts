...
const execGitCheckIgnore = async (
  ...
  const read = () => runGitRead
    ? runGitRead(cwd, () => execGit(args, cwd, { timeoutMs: GIT_CHECK_IGNORE_TIMEOUT_MS }), readOptions)
    : runWithGitExecutionScope(true, () => execGit(args, cwd, { timeoutMs: GIT_CHECK_IGNORE_TIMEOUT_MS }));
