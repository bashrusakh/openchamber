...
- `owned-process.ts`
  - Owns background child termination shared by Git and managed OpenCode. POSIX children have a separate process group, which receives SIGKILL after the grace period or root exit so a SIGTERM-resistant descendant cannot survive. Windows starts its hidden `taskkill` tree termination while the owned root is still live. Completion waits for stdio closure; a failed `taskkill`, or a root that closed before tree termination began, reports `ERR_PROCESS_TREE_TERMINATION` with descendant termination unconfirmed and retains the active-process registry/deactivation reset latch. It never retries `taskkill` by PID after root close, because Windows can reuse that PID. When the original pre-close `taskkill` succeeded but root close arrived after its bounded wait, the retained original close lifecycle reconciles the registry exactly once.
