...
Status freshness is directory-scoped. A failed `/session/status` request is not an authoritative empty snapshot: `global-session-status.ts` preserves the last known busy/retry entries and adds only that directory to `unavailableDirectories`. The flag is freshness, not an event gate, so live status events keep updating the index while it is set. A successful authoritative snapshot clears its own directory's flag and never another's, so a concurrent success for one directory cannot make another directory's preserved entries look fresh. `isSessionStatusFresh(sessionId, directory)` is the exported directory-freshness check, and its `directory` argument is required because absence from `statusById` means "last known idle", never "idle now" while the directory is unavailable.
