...
Status freshness is directory-scoped. A failed `/session/status` request is not an authoritative empty snapshot: `global-session-status.ts` preserves the last known busy/retry entries and adds only that directory to `unavailableDirectories`. The flag is freshness, not an event gate, so live status events keep updating the index while it is set. Any successful status fetch for the directory — monotonic or authoritative — calls `markDirectoryStatusFresh` and clears only its own flag, never another's, so a concurrent success for one directory cannot make another directory's preserved entries look fresh; the monotonic path clears freshness without gaining authority to lower busy/retry to idle. `isSessionStatusFresh(sessionId, directory)` is the exported directory-freshness check, and its `directory` argument is required because absence from `statusById` means "last known idle", never "idle now" while the directory is unavailable.

A transport disconnect or transport switch is transport-wide: `markTransportStatusUnavailable` adds every known directory (the `statusById` directories plus the child-store directories the sync provider passes, including idle-only ones), and each directory is freshened only by its own next snapshot, which the reconnect resync performs even for a directory with no active candidates. Freshness keys are normalized like the status entries.

A runtime replacement is a different boundary. `resetAppForRuntimeEndpointChange` calls `resetGlobalSessionStatus({ blockEventUpdates: true })`, destroying the previous runtime's status index and blocking `session.status`, `session.idle`, and `session.error` events until the new runtime's first authoritative snapshot re-enables them. `handleEvent` also drops payloads whose captured runtime key is no longer current, so late old-runtime status events cannot re-add confirmed activity to the new runtime.
