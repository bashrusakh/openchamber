...
- The shell widget's copy button writes the normalized visible text (`getShellClipboardText`) instead of the raw stream, so pasting does not replay carriage returns and erase sequences that collapse the output in a terminal. `copyTextToClipboard` restores focus and removes its fallback textarea even when `execCommand` throws.
