...
Archived folder scopes use `__archived__:<projectRoot>` and are materialized from complete global session coverage plus resolved worktree topology. `reconcileArchivedFolders` applies one idempotent batch per scope, preserves existing folder identity and collapse state, and leaves the current projection untouched while either authority is incomplete. Archived membership is read-only in the sidebar; session restore and deletion still use the normal folder cleanup path.
