...
`messageQueueStore.ts` hydrates the server-owned queue before migrating an older local projection. Reconnects run the same hydration, so persisted migrations, restores, and take acknowledgements can resume. A legacy message without `sendConfig` stays visible in `pendingLegacyMessages` until the current session's agent/model selection resolves to a valid provider and model. The store never guesses from a global last-used model. Failed enqueues, restores, and acknowledgements stay persisted for a later hydration; an empty `takeAll` is terminal and cannot be restored or re-enqueued.
...
`messageQueueStore.ts` keeps a queued message until its own send resolves, so between dispatch and resolution the entry is still visible to every reader. Dispatchers must therefore use the store's FIFO boundary: `markSending()` only claims the current head, and `getSendableQueue()` returns no items while any send for that target is unresolved. A composer submit must not filter `sendingIds` out and dispatch the tail, because that sends work twice or out of order (the window is seconds over a relay). `clearQueue()` retains in-flight entries for the same reason. `sendingIds` is deliberately not persisted: a restart has no in-flight sends, and a stale flag would strand a queued message. Desktop queues use the configured host id as runtime identity, not the current API URL, because an SSH reconnect allocates a new local forwarding port while the remote host remains the same.
...
A queued message is captured whole, so whoever delivers it sends exactly what the composer would have: `text` (the content with its agent mention stripped and `@file` mentions already resolved into `attachments`), `agentMention`, and `context` — every chip the composer had attached (inline comments, terminal selections, browser annotations, PR comments/checks, quotes, linked issue/PR/Linear references, pending synthetic parts) plus the skill instruction derived from the text. `QueuedContextPart` distinguishes attached items (restored to the chips when the message is edited) from derived instructions (re-derived on send, never restored) and from synthetic parts other surfaces handed the composer (restored as pending). Context is captured by `buildComposerContext` and delivered by `queuedContextToParts` (`components/chat/composer/submit/buildOutgoingMessage.ts`), the same functions the composer uses for its own send. Nothing is re-resolved at delivery: the server has no agent list, no confirmed mentions, and no draft store. Messages a previous build left in this browser are uploaded during hydration of a runtime. Accepted migrations leave persistence for that runtime, while failed migrations remain in `pendingServerEnqueues` and retry on a later hydration; `partialize` skips the server-owned queue projection after hydration. VS Code has no server and keeps the local queue with the foreground auto-send hook (`useQueuedMessageAutoSend`, enabled only there); `useMessageQueueHoldSync` tells the server to hold a session's queue while a UI-driven auto-review run is going. Hold cleanup keeps the captured runtime, session generation, client token, and sequence. Ordinary same-target hold mutations remain serialized, while runtime-switch releases use a dedicated non-blocking lane so an unresolved assertion cannot delay cleanup; failed releases retry in memory with bounded backoff. A newer token or sequence wins, and cleanup never releases a hold on the active runtime after a switch.
...
Chat composer drafts, confirmed mentions, inline-comment drafts, pending synthetic parts, and pinned sessions use the same runtime/directory/session ownership rule. Chat drafts use a bounded shared envelope and notify mounted composers when authoritative deletion clears their identity, preventing unmount autosave from resurrecting deleted text. Inline drafts enforce per-session, global-session, and serialized-byte bounds. Busy queue submissions capture their inline and synthetic context into that queue item's `additionalParts`, so overlapping sends cannot restore one item's context into another; direct sends still use the context handoff claim/restore path. Pins retain every valid composite key across runtimes without silent age/count eviction and are never pruned from the first startup list. Confirmed local deletion and routed deletion events clear immediately; after an authoritative baseline exists, a later complete omission also cleans persisted state. Ambiguous session-only legacy drafts and pins are not claimed.
...
Server queue hydration treats `complete: false` as a partial response and leaves the previous projection intact; only a complete snapshot may clear sessions omitted from it. Lifecycle records are authoritative for session incarnation, deletion, directory identity, and whether a restore must carry a take receipt. A take writes its operation receipt before the request and keeps it durable until the server acknowledges the receipt or the operation is invalidated by clear/delete. Reload recovery replays the take against its captured runtime and generation, then restores the full taken payload with the same receipt. Enqueue migration and normal enqueue both register a durable idempotency/removal record before POST; if removal wins after acceptance, the accepted server item is deleted rather than reapplied locally.
